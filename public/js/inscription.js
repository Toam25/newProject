$(function () {
        ...
        if (isNumberTotal === numberTotal) {
            $.ajax({
                ...
                success: function (data) {
                    if (data == "Enregistrer") {
                        $(".enregistrer").fadeIn().html('Le systeme vous dirigera vers Inother :)');
                        $('.ajax').css('display', 'none');
                        alert("Inscripation reussite, merci de vous connecter avec votre numéro téléphone et mots de passe");
    ...
    function recap() {
        ...
        return number1.toString() + alphabet[number2] + alphabet[number4].toLowerCase() + number3.toString();
    }
});