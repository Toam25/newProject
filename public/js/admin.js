...
=======
  ...
  //delete boutique
  $("body").on("click", ".supprimer_boutique", function (e) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      beforeSend: function () {},
      success: function (data) {
        that.parent("div").parents(".list_boutique").remove();
      },
      error: function () {
        toastr.error("Il y a un erreur");
      },
  ...
  $("body").on("click", ".article_edit", function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#view_referency").val(data.referency);
        $("#view_promo")
          .children('option[value="' + data.promo + '"]')
          .prop("selected", true);
        $("#view_promo_price").val(data.promo_price);
        $("#view_detail").val(data.description);
...
>>>>>>> refs/remotes/origin/master
  ...
  $("body").on("click", ".tab_men", function (e) {
    ...
    $(".add_option")
      .children('input[name="categorie_sante"]')
      .val(categorie_menu);
    $(".items-menu-selected").removeClass("items-menu-selected");
    $('button[name="' + categorie_menu + '"]')
      .parent("div")
      .addClass("items-menu-selected");
    $(".list_option").html("");
  ...
  $(".attibute_article_in").on("change", function () {
    ...
    $.ajax({
      ...
      dataType: "json",
      beforeSend: function () {
        $(".labelListOption").after(
          '<img class="loader_li" style="height: 14px" src="/images/images_default/ajax-loader.gif"/>'
        );
      },
      success: function (data) {
        let content = "";
        ...
        <div class="option_menu">` +
            element.name +
            `<button class="delete_option" value="` +
            element.id +
            `">
  ...
  $(".modifi_menu_in").on("click", function (e) {
    ...
    $(".name_menu").attr(
      "placeholder",
      $(this).parents(".items-menu").children(".name_menu_sante").text()
    );
    $(".add_option")
      .children('input[name="categorie_sante"]')
      .val(categorie_menu);
  ...
  $("body").on("submit", ".form_add_category ", function (e) {
    ...
    $.ajax({
      ...
      beforeSend: () => {
        $(".btn-submit").append(
          '<span class="loader_ajax" style="height: 20px;width: 20px;display: inline-block;"><img src="/images/images_default/ajax-loader.gif" style="height: 100%;width: 100%;"></span>'
        );
        $(".btn-submit").prop("disabled", true);
      },
      success: (data) => {
        ...
        let category =
          ...
          <button  data-parent = ` +
          data.id +
          ` class="btn btn-primary add_category">Ajout</button>
          </div>`;
              ...
              <div class="container_article col-xs-6 col-sm-3 col-md-2 col-lg-2">
                      <div class="list_produit">
                          ...
                          <input type="hidden" name="id_image" value="{{article.id}}">
                            ...
                            <img class="image_produit" src="/images/` +
            data.images[0] +
            `" alt="` +
            data.name +
            `">
                          <p class="name_article">` +
            data.name +
            `</p>
                            <span class="remove_article glyphicon glyphicon-remove"></span> 
                            <div>
                         <a class="btn btn-warning article_edit" href="/admin/article/` +
            data.id +
            `">Détail</a>
                         <a class="btn btn-danger article_delete" href="/admin/article/delete/` +
            data.id +
            `"">Supprimer</a>
                      </div>
                      </div>
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        for (let index = 0; index < results.length; index++) {
          ...
          sous_category +=
            `<option class="sous_categorie_menu_on_type" data-name="` +
            results[index].type +
            `" value="` +
            results[index].type +
            `" ` +
...
>>>>>>> refs/remotes/origin/master
  ...
  $(".sous_category").on("change", function (e) {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        $(".valeur_type").after(
          '<img style="height: 14px" src="/images/images_default/ajax-loader.gif "/>'
        );
      },
  ...
  $("._add_vote").on("submit", function (e) {
    ...
    $.ajax({
      ...
      success: (data) => {
        toastr.success("Enregistrer avec success");
        $(".container_all_image_vote").prepend(
          '<div  class="margin-top-5 image_vote col-xs-6 col-sm-4 col-md-2 col-lg-2"> <div class="container_image_for_header" style="background-color: ' +
            data.color +
            '"> <img name="' +
            data.id +
            '" alt="' +
            ...
            '" alt="' +
            data.apropos +
            '" class="image_for_body" src="/images/' +
            data.images +
            '"><input type="hidden" value="' +
            data.description +
            '"></div><button class="remove_vote btn btn-danger" data-idartcle="' +
            data.id +
            '" style="width: 100%;border: 0;border-radius: 0px !important;">Supprimer</button></div>'
        );
        $("#add_vote").modal("hide");
  ...
  $(".delete_header").on("click", function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#header_index")
          .children("img")
          .prop("src", "images/images_default/default_image.jpg");
  ...
  $(".show_header").on("click", function (e) {
    ...
    $("#_view_header")
      .children("img")
      .prop("src", $("#header_index").children("img").prop("src"));
  });
  ...
  $("body").on("submit", ".add_ess_article", function (e) {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        $(".name_menu").append(
          `<span><img class="loader_img_default"src="/assets/images/defaults/default_loader.gif"/>`
        );
      },
      success: function (data) {
  ...
  $(".ajout_ess_article_ev").on("click", function () {
    ...
    $(".modal-footer").html(
      `<button id="submitformarticle" type="submit" class="btn btn-success"> Enregistrer</button>`
    );
  });
    ...
    var types = [
      ...
      },
      {
        ...
        value: [
          ...
          "Shorties",
          "Strings",
          "Ensembles de Lingeries",
          "Lingeries Sculptantes",
          "Nuisettes",
          "Deshabillés",
          "Vêtements Thérmiques",
          "Soutiens Gorges",
        ],
      },
      {
      ...
      },
      {
        ...
        value: [
          "Fruits / corbeilles à pain",
          "Sets de table en bambou",
          "Boîte à épices",
        ],
      },
      {
        ...
        value: [
          "Cadre photo",
          "Support de pot de fleur",
          "Pots de fleur",
          "Décoration murale",
          "Objet design fer forge",
          "Embout",
          "Montre mural en fer forgé",
          "Bougeoir",
          "Photophore",
          "Applique & Luminaires",
          "Cornes de zébu",
...

      {
        ...
        value: [
          ...
          "Blush et poudres",
          "Fonds de teint et BB crème",
          "Rouges à lèvres",
          "Primers et correcteurs",
          "pilateurs sourcils",
          "Dépilatoires",
          "Accessoires maquillage ",
          "Anti-rides et anti-âges",
          "Masques et gommages",
          "Nettoyants et démaquillants",
          "Purifiants et matifiants",
          "Soins des lèvres et des yeux",
          "Crèmes",
          "Crème solaire",
          "Lotions",
          "Baume",
      ...
      },
      {
        ...
        value: ["HD Ready", "Full HD", "4K/UHD", "Accessoires"],
      },
      {
        name: "Son",
        value: [
  ...
  var containt_parameter = (categorie_menu) => {
    ...
    if (categorie_menu == "Outils de jardin") {
      (cible = "Jardinage"), (listOption = ["Outils de jardin"]);
    }
